...
```{r}
data <- read_excel("/Users/yingminhe/Downloads/allplayer_23-24.xlsx")
head(data)
```
...
Delving deeper, we turn the spotlight on individual performances using Jordan Poole’s shot charts for Points Per Shot (PPS) and expected Field Goal Percentage (xFG%) (see Screen Shots 1.24.49 PM and 1.25.02 PM). These visualizations map Poole's shooting efficiency across different court zones. The shading intensity reflects how his performance either exceeds or falls short of the league average. These insights can pinpoint not only the strengths and weaknesses of individual players but can also guide the team's development focus and game strategy.
